...
"#;
    ...
    keyring::store_otp_secret(test_username, test_secret)
        .expect("Failed to store test OTP secret");
    ...
    assert!(output.status.success(), "get-password should succeed with valid config and keyring");
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let stderr = String::from_utf8_lossy(&output.stderr);

    // Should output only the token (6-8 digits)
    assert!(stdout.len() >= 6 && stdout.len() <= 8, "Token should be 6-8 digits, got: '{}'", stdout);
    assert!(stdout.chars().all(|c| c.is_numeric()), "Token should contain only digits, got: '{}'", stdout);

    // Should have no stderr output for success case
    assert!(stderr.is_empty(), "Should have no stderr output on success, got: '{}'", stderr);
}
...
fn test_get_password_missing_config() {
    ...
    assert_eq!(output.status.code(), Some(2), "Should exit with code 2 for missing config");

    // Should have error message on stderr
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(!stderr.is_empty(), "Should have error message on stderr");
    assert!(stderr.contains("config") || stderr.contains("load"), "Error should mention config loading");

    // Should have no stdout output for error case
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.trim().is_empty(), "Should have no stdout output on config error");
}
